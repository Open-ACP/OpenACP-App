...
export function SettingsAgents({ workspacePath }: { workspacePath?: string }) {
  ...
  return (
    <div data-component="oac-settings" className="flex flex-col gap-6">
      ...
      {loading ? (
        <div className="text-sm-regular text-muted-foreground py-4">Loading agents...</div>
      ) : (
        <>
          {/* Installed */}
          {installed.length > 0 && (
            <div className="flex flex-col gap-1">
              <span className="text-2xs-regular text-foreground-weaker uppercase tracking-wider mb-1">Installed</span>
              {installed.map((agent) => (
                <AgentRow
                  key={agent.key}
                  agent={agent}
          ...
          {available.length > 0 && (
            <div className="flex flex-col gap-1">
              <span className="text-2xs-regular text-foreground-weaker uppercase tracking-wider mb-1">Available</span>
              {available.map((agent) => (
                <AgentRow
                  key={agent.key}
                  agent={agent}
          ...
          {installed.length === 0 && available.length === 0 && (
            <div className="text-sm-regular text-muted-foreground py-2">No agents found</div>
          )}
        </>
      )}
  ...
  return (
    <div className="flex items-center gap-3 py-2 px-3 rounded-md border border-border-weak/50">
      <div className="flex-1 min-w-0">
        <div className="text-md-medium text-foreground capitalize">{agent.name}</div>
        {agent.description && (
          <div className="text-sm-regular text-muted-foreground truncate">{agent.description}</div>
        )}
      </div>
      {agent.installed ? (
        onUninstall ? (
