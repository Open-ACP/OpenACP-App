...
const commands = {
  ...
  openPath: async (_p: string, _a: string | null) => {},
  getWslConfig: async () => ({ enabled: false }),
  setWslConfig: async (_c: any) => {},
  getDefaultServerUrl: async () => null as string | null,
  setDefaultServerUrl: async (_u: string | null) => {},
  getDisplayBackend: async () => null as string | null,
  setDisplayBackend: async (_b: string | null) => {},
  parseMarkdownCommand: async (md: string) => md,
  killSidecar: async () => {},
  checkAppExists: async (_n: string) => false,
}
...
const deepLinkEvent = "openacp:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__OPENACP__ ??= {}
  const pending = window.__OPENACP__.deepLinks ?? []
  window.__OPENACP__.deepLinks = [...pending, ...urls]
...
const createPlatform = (): Platform => {
  ...
  const wslHome = async () => {
    if (os !== "windows" || !window.__OPENACP__?.wsl) return undefined
    return commands.wslPath("~", "windows").catch(() => undefined)
  }

  const handleWslPicker = async <T extends string | string[]>(result: T | null): Promise<T | null> => {
    if (!result || !window.__OPENACP__?.wsl) return result
  ...
  return {
      ...
      await Promise.resolve()
        .then(() => {
          const notification = new Notification(title, {
            body: description ?? "",
            icon: "https://openacp.com/favicon-96x96-v3.png",
          })
    ...
    getWslEnabled: async () => {
      ...
      return window.__OPENACP__!.wsl ?? false
    },
...
render(() => {
  ...
  // Fetch OpenACP server info from Rust (reads ~/.openacp/api.port + api-secret)
  ...
  // Build the server connection from OpenACP server info
  const servers = () => {
    const data = sidecar()
    if (!data) return []
    const http = {
